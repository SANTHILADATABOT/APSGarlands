import '../sass/galleries-picker.scss';
...
export default class {
    constructor(options) {
        this.options = _.merge({
            ...
            title: trans('galleries::galleries.file_manager.title'),
            message: trans('galleries::messages.image_has_been_added'),
        }, options);
    ...
    appendModalToBody() {
        if ($('.galleries-picker-modal').length === 1) {
            return;
        }
    ...
    showModal() {
        let modal = $('.galleries-picker-modal').modal('show');
    ...
    selectGalleries() {
        this.frame.contents().find('.table').on('click', '.select-galleries', (e) => {
            e.preventDefault();
            ...
            if (this.options.multiple) {
                ...
                $('.galleries-picker-modal').modal('hide');
            }
        });
    }
    ...
    getModal() {
        return `
            <div class="galleries-picker-modal modal fade" role="dialog">
